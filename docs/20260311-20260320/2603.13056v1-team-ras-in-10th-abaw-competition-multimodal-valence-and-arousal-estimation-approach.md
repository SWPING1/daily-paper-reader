...
score: 8.0
evidence: 野外环境下的多模态效价和唤醒度估计
tldr: 本研究针对野外环境下连续情感识别（效价和唤醒度）面临的复杂挑战，提出了一种整合人脸、行为和音频的多模态估算方法。该方法利用GRADA提取人脸特征，通过Qwen3-VL和Mamba捕捉行为动态，并结合WavLM处理音频。研究重点探索了有向跨模态混合专家融合及可靠性感知融合策略，旨在提高复杂场景下的识别精度。
selection_source: fresh_fetch
motivation: 旨在解决野外环境下因光照、遮挡和个体差异导致的连续情感识别（效价与唤醒度）不准确问题。
method: 融合了基于GRADA的人脸特征、基于Qwen3-VL与Mamba的行为特征以及WavLM音频特征，并采用混合专家（MoE）等策略进行多模态融合。
result: 在Aff-Wild2数据集的开发集上，该多模态融合方案实现了0.658的一致性相关系数（CCC）。
conclusion: 实验结果表明，结合先进的视觉语言模型与创新的多模态融合策略能有效提升野外环境下的情感识别性能。
---

## 摘要
在野外（ITW）条件下，基于效价（Valence）和唤醒度（Arousal）的连续情感识别由于外观、头部姿态、光照、遮挡以及受试者特定的情感表达模式存在巨大差异，仍然是一个具有挑战性的问题。我们提出了一种用于野外环境下效价-唤醒度估计的多模态方法。该方法结合了三种互补的模态：面部、行为和音频。面部模态依赖于基于GRADA的帧级嵌入和基于Transformer的时间回归。我们使用Qwen3-VL-4B-Instruct从视频片段中提取行为相关信息，并利用Mamba对片段间的时间动态进行建模。音频模态依赖于带有注意力统计池化的WavLM-Large，并包含一个跨模态过滤阶段，以减少不可靠或非语音片段的影响。为了融合模态，我们探索了两种融合策略：一种是通过自适应加权学习模态间交互的有向跨模态混合专家（MoE）融合策略，以及一种在帧级结合视觉特征并将音频作为补充上下文的可靠性感知视听融合策略。实验结果基于第10届野外情感行为分析（ABAW）竞赛协议在Aff-Wild2数据集上进行评测。实验表明，所提出的多模态融合策略在Aff-Wild2开发集上达到了0.658的一致性相关系数（CCC）。
