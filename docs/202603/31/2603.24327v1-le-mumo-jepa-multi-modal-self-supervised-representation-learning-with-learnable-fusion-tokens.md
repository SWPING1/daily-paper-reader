...
title_zh: Le MuMo JEPA：具有可学习融合 Token 的多模态自监督表示学习
...
motivation: 旨在解决当前自监督学习方法大多仅处理单一模态，从而无法利用异构传感器提供的互补结构信息的问题。
method: 采用可学习的融合标记作为模态间的潜在瓶颈，并结合剪枝融合策略与SIGReg正则化来学习多模态联合表示。
result: 在多个自动驾驶基准测试中，该方法在目标检测、深度估计和分割任务上均取得了领先的性能与效率权衡。
conclusion: Le MuMo JEPA通过高效的跨模态注意力机制和瓶颈设计，为多模态自监督表示学习提供了一个高性能且低功耗的解决方案。
---

## 摘要
自监督学习已成为一种无需手动标注即可学习视觉表示的强大范式，但大多数方法仍仅针对单一模态，因此错失了异构传感器提供的互补结构。我们提出了 Le MuMo JEPA，这是一个从 RGB 图像及其对齐的伴随模态中学习统一表示的自监督框架。在我们的自动驾驶实验中，第二种模态是与相机对齐的 LiDAR 深度；我们还在 Teledyne FLIR ADAS 基准测试中评估了 RGB-热成像训练和迁移。我们的方法通过学习融合 Token（fusion tokens）将 LeJEPA 扩展到多模态设置，这些 Token 在共享 Transformer 内部的模态特定 patch stem 之间充当潜瓶颈（latent bottleneck）。我们的默认模型采用了一种剪枝融合策略：在初始跨模态注意力层之后，丢弃模态特定的 Token，迫使跨模态信息进入共享的融合 Token 网格，作为一种高效的潜瓶颈，随后对联合多模态 CLS 嵌入应用草图各向同性高斯正则化（SIGReg）。在 Waymo 数据集上，Le MuMo JEPA 在从零开始训练的多模态基准模型中，在下游 patch 探测任务上表现出最强的性能-效率权衡，改进了 CenterNet 检测和稠密深度，同时在分割任务上保持了竞争力。在 nuScenes 的从零开始训练中，Le MuMo JEPA 依然是最强的模型，并且在 FLIR 上也取得了最佳结果，尤其是在经过 Waymo 初始化的微调之后。在我们的研究中，它还以显著更低的计算量、内存占用和预估训练时间，保持了最佳的整体准确率-效率平衡。
