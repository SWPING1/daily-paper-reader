...
motivation: 旨在解决大语言视觉模型中密集视觉Token带来的巨大计算和内存开销，同时避免昂贵的重新训练过程。
method: 将剪枝视为子空间重构问题，采用残差能量引导的贪婪扩展策略，并结合文本指令相关性来保留关键Token。
result: 在LLaVA-1.5、LLaVA-NeXT和Qwen2.5-VL等多个模型上的实验表明，该方法在性能和效率上均优于现有剪枝技术。
conclusion: ResPrune为提升LVLM推理效率提供了一种高效、通用且无需训练的视觉Token压缩新方案。
---
