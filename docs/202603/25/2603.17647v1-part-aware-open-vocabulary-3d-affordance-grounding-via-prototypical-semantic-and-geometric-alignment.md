...
motivation: 现有的3D功能区域定位方法在处理开放词汇、细粒度几何对齐及部件级语义一致性方面仍存在显著挑战。
method: 提出一种结合LLM生成部件感知指令、原型聚合（APA）及物体内关系建模（IORM）的两阶段跨模态对齐框架。
result: 在多个基准测试上的实验结果表明，该方法在开放词汇3D功能区域定位任务中表现优异，显著超越了现有方法。
conclusion: 通过语义增强与几何原型对齐的结合，该框架有效提升了模型对3D物体功能区域的理解与定位精度。
---
