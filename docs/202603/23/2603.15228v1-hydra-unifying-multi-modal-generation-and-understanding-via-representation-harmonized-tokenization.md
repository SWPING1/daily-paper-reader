...
## 摘要
统一多模态模型（Unified Multimodal Models）难以弥合视觉理解所需的抽象表示与生成所需的详细原语（primitives）之间的根本鸿沟。现有方法通常通过采用解耦编码器、在 VAE 之上堆叠表示编码器或利用离散量化来进行折中。然而，这些方法往往会破坏信息连贯性并导致优化冲突。为此，我们基于视觉建模应从生成演进到理解的洞察，提出了 HYDRA-TOK，一种表示协调的纯 ViT。HYDRA-TOK 将标准骨干网络重构为一个渐进式学习器，从捕获结构保持原语的 Gen-ViT 过渡到用于语义编码的 Sem-ViT。关键在于，这种过渡由生成-语义瓶颈（Generation-Semantic Bottleneck, GSB）介导，它将特征压缩到低维空间以过滤噪声从而实现鲁棒合成，随后恢复维度以增强复杂的语义理解。在此基础上，我们提出了 HYDRA，一个在单一参数空间内集成感知与生成的原生统一框架。大量实验证明 HYDRA 达到了新的 SOTA 水平。它在视觉重建方面树立了基准（rFID 0.08），并在 GenEval (0.86)、DPG-Bench (86.4) 和 WISE (0.53) 上实现了顶尖的生成性能，同时在八个具有挑战性的理解基准测试中，比之前的原生 UMM 平均高出 10.0 分。
