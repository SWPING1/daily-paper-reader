...
score: 7.0
evidence: 模态间隙与红外样本稀缺
tldr: 针对可见光-红外行人重识别（VI-ReID）中模态差异大和分布偏移的问题，本文提出了一种双向交互变换网络（BIT）。BIT 摒弃了传统的刚性特征对齐，采用基于匹配的策略，通过编码器-解码器架构显式建模图像对之间的交互，并引入双向特征整合与查询感知评分。作为首个引入成对匹配驱动交互的方法，BIT 在多个基准测试上达到了 SOTA 性能。
selection_source: fresh_fetch
motivation: 现有方法往往忽略了模态间复杂且隐式的关联，且在红外样本稀缺的分布偏移情况下表现不佳。
method: 提出 BIT 网络，利用编码器-解码器架构进行双向特征整合和查询感知评分，以增强跨模态对应关系。
result: 在多个主流基准数据集上的实验结果表明，BIT 显著提升了识别精度，达到了当前最先进的水平。
conclusion: BIT 通过显式的成对匹配驱动交互，有效地缓解了模态鸿沟，为 VI-ReID 任务提供了一种高效的新方案。
---

## 摘要
可见光-红外行人重识别（VI-ReID）是一项具有挑战性的检索任务，原因在于可见光图像与红外图像之间存在巨大的模态差异。虽然现有方法试图通过在共享嵌入空间中学习模态不变特征来弥合这一差距，但它们往往忽略了模态之间复杂且隐式的相关性。在分布偏移的情况下，这种局限性变得更加严重，因为红外样本通常远少于可见光样本。为了应对这些挑战，我们提出了一种名为双向交互变换（BIT）的新型网络。BIT 不依赖于僵化的特征对齐，而是采用一种基于匹配的策略，显式地建模可见光与红外图像对之间的交互。具体而言，BIT 采用编码器-解码器架构，其中编码器提取初步的特征表示，解码器执行双向特征集成和查询感知评分，以增强跨模态对应关系。据我们所知，BIT 是首个在 VI-ReID 中引入这种成对匹配驱动交互的方法。在多个基准测试上的广泛实验表明，我们的 BIT 达到了最先进的性能，突显了其在 VI-ReID 任务中的有效性。
