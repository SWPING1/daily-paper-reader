...
## 摘要
多模态3D视觉-语言模型在多种3D任务中表现出强大的泛化能力，但在领域偏移（domain shifts）下其性能仍会显著下降。这激发了近期对测试时自适应（TTA）的研究，该技术使模型能够利用测试时数据进行在线自适应。在现有的TTA方法中，基于缓存的机制被广泛采用，用于在在线预测细化中利用先前观察到的样本。然而，它们仅存储有限的历史信息，导致随着测试流的演进，信息逐渐丢失。此外，它们的预测对数几率（logits）采用启发式融合，使得自适应过程不稳定。为了解决这些局限性，我们提出了 BayesMM，一个用于测试时点云分析的多模态贝叶斯分布学习框架。BayesMM 将每个类别的文本先验和流式视觉特征建模为高斯分布：文本参数源自语义提示，而视觉参数则随到达的样本在线更新。这两种模态通过贝叶斯模型平均进行融合，该方法根据后验证据自动调整它们的贡献，从而产生统一的预测，无需训练即可持续适应不断演进的测试时数据。在多个点云基准测试上的广泛实验表明，BayesMM 在分布偏移下保持了鲁棒性，平均性能提升超过 4%。
