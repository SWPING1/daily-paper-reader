...
title_zh: EmoTaG：基于高斯泼溅与少样本个性化的情感感知说话人脸合成
...
tldr: 本研究针对少样本3D说话人脸合成中情感表达不自然和几何不稳定的挑战，提出了EmoTaG框架。该方法基于3D高斯泼溅技术，通过在结构化的FLAME参数空间内建模运动，并结合门控残差运动网络（GRMN）捕捉音频中的情感韵律与头部姿态，实现了仅需数秒视频即可生成高保真、情感丰富且运动稳定的个性化说话人脸。
selection_source: fresh_fetch
motivation: 旨在解决现有少样本说话人脸合成方法在处理丰富表情时出现的几何畸变以及音频与情感表达不匹配的问题。
method: 将运动预测重构为FLAME参数空间以引入几何先验，并设计门控残差运动网络来融合音频情感韵律与面部运动线索。
result: 实验结果显示，EmoTaG在情感表达力、口型同步、视觉真实感和运动稳定性等多个维度上均优于现有最先进方法。
conclusion: 通过结合结构化几何先验与有效的情感运动建模，EmoTaG显著提升了少样本个性化3D说话人脸合成的质量与表现力。
---

## 摘要
音频驱动的 3D 说话人脸合成在神经辐射场（NeRF）和 3D 高斯泼溅（3DGS）的推动下取得了飞速发展。通过利用丰富的预训练先验，少样本方法仅需几秒钟的视频即可实现即时个性化。然而，在富有表现力的面部运动下，现有的少样本方法往往面临几何不稳定和音频-情感不匹配的问题，这凸显了对更有效的情感感知运动建模的需求。在这项工作中，我们提出了 EmoTaG，这是一个基于“预训练-适配”（Pretrain-and-Adapt）范式的少样本情感感知 3D 说话人脸合成框架。我们的核心见解是在结构化的 FLAME 参数空间中重新表述运动预测，而不是直接对 3D 高斯进行变形，从而引入显式的几何先验以提高运动稳定性。在此基础上，我们提出了门控残差运动网络（GRMN），该网络能够从音频中捕捉情感韵律，同时补充音频中缺失的头部姿态和上脸部线索，从而实现富有表现力且连贯的运动生成。大量实验表明，EmoTaG 在情感表达力、口型同步、视觉逼真度和运动稳定性方面均达到了最先进的性能。
