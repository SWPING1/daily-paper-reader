...
tldr: 针对点级弱监督时序情感定位（P-WTSL）中情感边界不精确的挑战，本文提出人脸引导情感边界增强网络（FSENet）。该框架通过引入人脸引导情感发现模块整合面部特征，利用点感知语义对比策略强化边界识别，并设计边界感知伪标签生成方法将稀疏标注转化为平滑监督信号。实验证明，FSENet在多种监督设置下均达到领先水平，显著提升了情感定位的准确性。
selection_source: fresh_fetch
motivation: 旨在解决点级弱监督时序情感定位中由于标注稀疏导致的情感边界识别不准确问题。
method: 提出FSENet框架，结合人脸特征引导、对比学习语义判别以及边界感知伪标签生成技术。
result: 在基准数据集上取得了当前最先进的性能，且在全监督、视频级和点级弱监督设置下均表现优异。
conclusion: 该研究证明了利用细粒度面部信息和对比学习策略能有效增强弱监督视频情感定位的边界精度和泛化性。
---
