...
tldr: 本研究针对可穿戴设备中人体活动识别（HAR）面临的隐私泄露和标注数据稀缺问题，提出了一种名为CFD-HAR的条件特征解耦技术。该方法通过在潜空间中分离活动特征与敏感属性，实现了细粒度的用户可控隐私过滤。研究对比了CFD与基于自编码器的少样本学习方法，分析了它们在隐私保护、数据效率及边缘部署方面的优劣，为构建可信物联网智能系统提供了方向。
selection_source: fresh_fetch
motivation: 旨在解决移动和可穿戴设备在进行活动识别时，如何在保护用户敏感隐私的同时保持高识别性能。
method: 提出基于条件特征解耦的表示学习框架，通过在潜空间中解耦活动与敏感信息来实现动态隐私过滤。
result: 实验表明CFD-HAR能提供显式且可调的隐私控制，而传统的少样本学习方法虽效率高但缺乏隐私保护。
conclusion: 论文指出单一范式难以兼顾隐私与效率，未来研究应致力于开发集成隐私保护、少样本适应性和鲁棒性的统一框架。
---

## 摘要
现代可穿戴和移动设备配备了惯性测量单元 (IMU)。运行在这些设备上的人体活动识别 (HAR) 应用利用基于机器学习的数据驱动技术来处理此类传感器数据。然而，基于传感器数据的 HAR 部署面临两个关键挑战：根据用户的隐私偏好保护嵌入在传感器数据中的敏感用户信息，以及在有限的标注样本下保持高识别性能。本文提出了一种通过基于特征解耦的细粒度表示学习来实现用户可控隐私的技术，用于动态隐私过滤。我们还将该技术的有效性与使用基于自动编码器表示学习的小样本 HAR 进行了对比。我们分析了它们的架构设计、学习目标、隐私保证、数据效率以及对边缘物联网 (IoT) 部署的适用性。研究表明，基于 CFD 的 HAR 通过在潜空间中分离活动属性和敏感属性，提供了显式且可调的隐私保护控制；而基于自动编码器的小样本 HAR 虽然具有卓越的标签效率和轻量级适应性，但缺乏内在的隐私保护机制。我们进一步研究了这两种方法在持续物联网环境下的安全影响，重点分析了它们在表示泄露和嵌入级攻击敏感性方面的差异。分析显示，单一范式都无法完全满足下一代物联网 HAR 系统的新兴需求。最后，我们概述了旨在联合优化隐私保护、小样本适应性和鲁棒性的统一框架研究方向，以实现可信的物联网智能。
