...
## 摘要
多视图学习的主要目标是融合多种特征以全面地描述数据。大多数先前的研究隐含地假设不同视图具有相似的维度。然而在实践中，不同视图之间往往存在严重的维度差异，从而导致不平衡的多视图学习问题。例如，在情感识别任务中，视频帧的维度通常达到 $10^6$，而生理信号仅包含 $10^1$ 个维度。现有方法针对该问题通常面临两个主要挑战：（1）它们往往偏向高维数据，忽略了低维视图；（2）在极端的维度不平衡下，它们难以有效地对齐表示，这给低维表示引入了严重的冗余。为了解决这些问题，我们提出了自适应多视图稀疏学习（AdaMuS）框架。首先，为了防止忽略低维视图的信息，我们构建了特定视图的编码器，将其映射到统一的维度空间。鉴于将低维数据映射到高维空间通常会导致严重的过拟合，我们设计了一种无参数剪枝方法，以自适应地移除编码器中的冗余参数。此外，我们提出了一种稀疏融合范式，能够灵活地抑制冗余维度并有效地对齐各个视图。此外，为了学习具有更强泛化能力的表示，我们提出了一种自监督学习范式，通过构建相似度图来获取监督信息。在合成玩具数据集和七个真实世界基准测试上的广泛评估表明，AdaMuS 在分类和语义分割任务中均能持续取得优异的性能，并表现出强大的泛化能力。
