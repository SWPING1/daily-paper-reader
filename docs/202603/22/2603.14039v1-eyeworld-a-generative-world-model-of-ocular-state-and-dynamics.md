...
score: 7.0
evidence: 用于跨模态转换和质量鲁棒增强的生成式世界模型
tldr: 针对眼科决策依赖多模态及时间序列细微病变特征且现有模型鲁棒性不足的问题，本文提出EyeWorld。这是一个生成式世界模型，将眼睛视为部分观测的动力系统，通过学习跨模态的稳定潜状态，实现了精细解析、跨模态翻译及质量增强。该模型还支持时间条件的病变预测，为医学多模态解释和预后模拟提供了统一框架。
selection_source: fresh_fetch
motivation: 现有的医学基础模型多为静态，在面对模态切换、采集偏差及随访预测时表现不佳。
method: 将眼部建模为基于临床影像的部分观测动力系统，学习跨模态共享的稳定潜状态，并引入纵向监督进行时间条件的状态转换。
result: 在单一框架下实现了结构保留的跨模态翻译、图像增强及临床意义上的病变进展预测。
conclusion: EyeWorld 通过从静态表示学习转向显式动力学建模，为稳健的多模态医学影像解释和预后模拟提供了新范式。
---
