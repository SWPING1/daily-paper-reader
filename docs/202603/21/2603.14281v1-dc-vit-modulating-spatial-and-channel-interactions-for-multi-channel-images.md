...
tldr: 针对多通道成像（MCI）中通道配置异构导致的特征稀释问题，本文提出了解耦视觉Transformer（DC-ViT）。该模型通过解耦自注意力（DSA）机制，将空间内部结构建模与跨通道信息整合分离，有效保留了通道特定的语义信息。此外，引入解耦聚合（DAG）模块以学习任务相关的通道权重。实验证明，DC-ViT在多个MCI基准测试中均优于现有方法，显著提升了多通道数据的表征能力。
selection_source: fresh_fetch
motivation: 现有的多通道ViT在统一注意力空间中处理所有通道，容易导致特征稀释并丢失关键的通道特定语义。
method: 提出解耦自注意力（DSA）来分离空间与通道维度的更新，并利用解耦聚合（DAG）自适应地整合不同通道的重要性。
result: 在三个多通道成像基准测试中，DC-ViT 相比现有的多通道 ViT 方法取得了持续且显著的性能提升。
