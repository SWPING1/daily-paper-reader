...
tldr: 针对视觉语言模型中跨模态对齐和可解释性不足的问题，本文提出了SCoCCA框架。该方法结合了典型相关分析（CCA）与稀疏概念分解，旨在消除模态间隙并提取具有语义意义的稀疏概念。通过揭示CCA与InfoNCE目标的内在联系，SCoCCA在无需额外训练的情况下实现了高效的跨模态对齐，并在概念发现、消融和语义操作任务中达到了最先进的性能，为多模态模型的可解释性提供了新路径。
selection_source: fresh_fetch
motivation: 现有的概念解释方法多局限于单模态且忽视了视觉语言模型中的模态间隙，限制了对跨模态交互的深入理解。
method: 提出SCoCCA框架，利用CCA对齐跨模态嵌入，并引入稀疏性约束进行概念分解，以生成更具辨别力和解耦性的语义成分。
result: 实验证明SCoCCA在概念发现、重构以及概念消融等语义操作任务上均取得了当前最优的性能表现。
conclusion: SCoCCA成功将概念解释扩展到多模态领域，为理解和操纵视觉语言模型的内部推理提供了高效且无需训练的工具。
---

## 摘要
解释视觉语言模型的内部推理对于在安全关键领域部署人工智能至关重要。基于概念的可解释性通过具有语义意义的组件来表示模型的行为，提供了一个符合人类认知的视角。然而，现有方法在很大程度上局限于图像，忽略了跨模态交互。文本-图像嵌入（例如 CLIP 生成的嵌入）存在模态间隙，即视觉和文本特征遵循不同的分布，从而限制了可解释性。典型相关分析 (CCA) 提供了一种原则性的方法来对齐来自不同分布的特征，但尚未被用于多模态概念级分析。我们证明了 CCA 和 InfoNCE 的目标函数密切相关，因此优化 CCA 隐含地优化了 InfoNCE，从而提供了一种简单的、无需训练的机制来增强跨模态对齐，且不影响预训练的 InfoNCE 目标。受此观察启发，我们将基于概念的可解释性与 CCA 相结合，引入了 Concept CCA (CoCCA)，这是一个在实现可解释概念分解的同时对齐跨模态嵌入的框架。我们进一步对其进行扩展，提出了稀疏概念 CCA (SCoCCA)，它通过强制稀疏性来产生更解耦且更具辨别力的概念，从而促进了更好的激活、消融和语义操作。我们的方法将基于概念的解释推广到多模态嵌入，并在概念发现方面实现了最先进的性能，这在重建和概念消融等操作任务中得到了证明。
