...
## 摘要
语音情感识别 (SER) 在推动人机交互发展方面发挥着关键作用。由于注意力机制能够捕捉长距离依赖关系并强调显著信息，它已成为建模情感语音的主流方法。然而，标准自注意力机制存在二次方计算和内存复杂度的问题，限制了其可扩展性。在这项工作中，我们对用于 SER 的优化注意力机制进行了系统基准测试，包括 RetNet、LightNet、GSA、FoX 和 KDA。在两个版本的 MSP-Podcast 基准数据集上的实验表明，虽然标准自注意力在各测试集中实现了最强的识别性能，但高效的注意力变体显著提高了可扩展性，将推理延迟和内存占用降低了多达一个数量级。这些结果突显了准确率与效率之间的关键权衡，为设计可扩展的 SER 系统提供了实用见解。
