...
tldr: "评估AI配音内容通常依赖昂贵的人工评分（MOS）。本研究提出一种分层多模态架构，整合音频（音色、韵律）、视频（表情、场景）和文本（语义）特征，通过层级融合捕捉细粒度感知信息。利用LoRA适配器和主动学习优化的代理评分进行高效训练。实验表明，该方法在印地语-英语双向配音数据集上实现了与人类感知的高度一致性（PCC > 0.75），为AI配音自动评估提供了可扩展的解决方案。"
selection_source: fresh_fetch
motivation: 针对AI配音评估维度复杂且人工评分成本高昂的问题，探索一种能模拟人类感知的自动化评估方法。
method: 提出一种分层跨模态融合架构，结合音视频及文本特征，并利用LoRA适配器和主动学习生成的代理评分进行训练。
result: "该模型在12,000个双向配音片段上表现优异，与人类平均意见得分的相关性系数（PCC）超过0.75。"
conclusion: 分层多模态融合能有效预测人类对AI配音的感知质量，为大规模自动化评估提供了可靠且高效的路径。
---
