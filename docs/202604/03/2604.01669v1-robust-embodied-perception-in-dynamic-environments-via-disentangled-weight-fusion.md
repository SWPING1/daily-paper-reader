...
tldr: 针对具身感知系统在动态环境中面临的分布偏移和灾难性遗忘问题，本文提出一种无需领域ID和样本存储的增量学习框架。该框架通过解耦表示机制消除环境风格干扰并提取共享语义特征，结合权重融合策略在参数空间动态整合新旧知识。实验证明，该方法在无样本条件下显著提升了模型的泛化能力和环境适应性，性能优于现有主流方法。
selection_source: fresh_fetch
motivation: 现有的增量学习方法依赖领域ID且易受环境噪声干扰，导致在未知动态场景中泛化能力差并存在严重的灾难性遗忘。
method: 提出一种解耦表示机制以提取跨场景语义特征，并利用权重融合策略在不存储历史数据的情况下实现新旧环境知识的动态集成。
result: 在多个基准数据集上的实验表明，该方法在完全无需样本和领域ID的设定下，显著减少了遗忘并取得了领先的准确率。
conclusion: 该研究为具身感知系统在复杂动态环境下的持续学习提供了一种高效且鲁棒的解决方案。
---
