...
## 摘要
我们针对高维贝叶斯反问题提出了一种结构化先验，该先验基于解耦深度生成模型，其潜在空间被划分为与已知且可解释的物理参数对齐的辅助变量，以及捕捉剩余未知变异性的残差变量。这产生了一个层级先验，其中可解释的坐标承载了领域相关的不确定性，而残差坐标则保留了深度生成模型的灵活性。通过对生成器进行线性化，我们刻画了诱导先验协方差，并推导了后验在潜在变量中表现出近似块对角结构的条件，从而阐明了表示层级的解耦何时能转化为反问题中不确定性的分离。我们构建了由此产生的潜在空间反问题，并使用最大后验（MAP）估计和马尔可夫链蒙特卡罗（MCMC）采样进行求解。在椭圆偏微分方程（PDE）反问题（如电导率识别和源项识别）中，该方法在模型设定正确时能达到理想高斯过程先验的性能，并在先验设定错误时提供显著改进，同时能够恢复可解释的物理参数并产生空间校准的不确定性估计。
