...
title_zh: 针对空间对齐图像的置换不变离散表示学习研究
...
tldr: 本研究探讨了空间对齐图像的置换不变离散表示学习。针对传统VQ-VAE编码依赖位置且上下文纠缠的问题，提出了PI-VQ模型，强制潜码不携带位置信息。通过引入基于最优二分匹配的“匹配量化”算法，显著提升了瓶颈容量。该方法使模型能捕捉全局语义特征，支持无需先验的直接插值采样，在CelebA等数据集上实现了高质量的图像合成。
selection_source: fresh_fetch
motivation: 传统的离散表示学习高度依赖空间位置信息，导致采样时需要复杂的自回归或扩散先验来建模编码间的依赖关系。
method: 提出PI-VQ模型，通过置换不变约束和基于最优二分匹配的“匹配量化”算法，使离散编码仅捕获全局语义特征。
result: 匹配量化将有效瓶颈容量提升了3.5倍，且模型在CelebA和FFHQ等数据集上展现出优秀的插值能力和合成指标。
conclusion: 置换不变的离散表示能有效分离语义与位置信息，为无需复杂先验的图像合成和可解释表示学习提供了新方向。
---

## 摘要
矢量量化方法（如 VQ-VAE、VQ-GAN）学习图像的离散神经表示，但这些表示本质上是位置相关的：编码在空间上排列且在上下文中纠缠，在采样时需要自回归或基于扩散的先验来建模其依赖关系。在这项工作中，我们探讨了对于空间对齐数据的离散表示，位置信息是否是必要的。我们提出了置换不变矢量量化自编码器（PI-VQ），其中潜码被约束为不携带任何位置信息。我们发现，这种约束促使编码捕获全局语义特征，并允许在无需学习先验的情况下直接在图像之间进行插值。为了解决置换不变表示信息容量降低的问题，我们引入了匹配量化（matching quantization），这是一种基于最优二分匹配的矢量量化算法，相对于朴素的最近邻量化，它将有效瓶颈容量提高了 3.5 倍。所学编码的组合结构进一步实现了基于插值的采样，允许通过单次前向传递合成新图像。我们在 CelebA、CelebA-HQ 和 FFHQ 数据集上评估了 PI-VQ，在合成图像的精度、密度和覆盖率指标上均取得了具有竞争力的结果。我们讨论了无位置表示中固有的权衡，包括潜码的可分离性和可解释性，并指出了未来研究的多个方向。
