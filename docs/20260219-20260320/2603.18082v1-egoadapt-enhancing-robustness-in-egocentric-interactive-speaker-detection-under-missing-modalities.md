...
## 摘要
TTM（Talking to Me）任务是理解人类社交互动的关键组成部分，旨在确定谁正在与摄像机佩戴者进行对话。由于视觉数据缺失、忽视头部朝向的作用以及背景噪声，传统模型在现实场景中经常面临挑战。本研究通过引入 EgoAdapt 解决了这些局限性，这是一个专为缺失模态下鲁棒的第一视角“Talking to Me”说话人检测而设计的自适应框架。具体而言，EgoAdapt 包含三个关键模块：(1) 视觉说话人目标识别 (VSTR) 模块，捕捉作为非语言线索的头部朝向和作为语言线索的唇部运动，从而实现对语言和非语言信号的全面解释以解决 TTM 问题，这使其区别于仅关注检测说话状态的任务；(2) 并行权值共享音频 (PSA) 编码器，用于在噪声环境中增强音频特征提取；以及 (3) 视觉模态缺失感知 (VMMA) 模块，该模块估计每一帧中各模态的存在或缺失，以动态调整系统响应。在 Ego4D 数据集的 TTM 基准上进行的全面评估表明，EgoAdapt 实现了 67.39% 的平均精度均值 (mAP) 和 62.01% 的准确率 (Acc)，在准确率上显著超过现有最先进方法 4.96%，在 mAP 上超过 1.56%。
