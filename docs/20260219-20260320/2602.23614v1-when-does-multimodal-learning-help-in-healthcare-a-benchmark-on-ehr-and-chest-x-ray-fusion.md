...
title_zh: 多模态学习何时在医疗保健中发挥作用？关于电子健康记录（EHR）与胸部X射线（CXR）融合的基准研究
...
result: 多模态融合在信息互补时能提升性能，但易受模态不平衡影响，且在模态缺失时性能显著下降，同时并未自动改善公平性。
conclusion: 提供了医疗多模态学习的实践指导，并发布了开源工具包CareBench以支持开发更有效、可靠且公平的临床部署系统。
---

## 摘要
机器学习在推进临床决策支持方面具有前景，但在实践中，特别是在模态缺失和公平性约束下，多模态学习何时真正发挥作用仍不清楚。在这项工作中，我们基于 MIMIC-IV 和 MIMIC-CXR 的标准化队列，对电子健康记录（EHR）和胸部 X 射线（CXR）之间的多模态融合进行了系统性基准测试，旨在回答四个基本问题：多模态融合何时能提高临床预测性能、不同融合策略的比较如何、现有方法对模态缺失的鲁棒性如何，以及多模态模型是否实现了算法公平性。我们的研究揭示了几个关键见解。当模态完整时，多模态融合能提高性能，且增益主要集中在需要 EHR 和 CXR 互补信息的疾病上。虽然跨模态学习机制捕捉到了超越简单拼接的具有临床意义的依赖关系，但 EHR 丰富的时序结构引入了强烈的模态不平衡，仅靠架构复杂性无法克服。在现实的缺失情况下，除非模型经过显式设计以处理不完整输入，否则多模态带来的收益会迅速下降。此外，多模态融合并不能本质上提高公平性，子群体间的差异主要源于不同人口统计学群体之间不平等的敏感性。为了支持可复现和可扩展的评估，我们进一步发布了一个灵活的基准测试工具包，支持新模型和数据集的即插即用集成。总之，这项工作为多模态学习何时有效、何时失效以及原因提供了可操作的指导，为开发既有效又可靠的临床可部署多模态系统奠定了基础。开源工具包可在 https://github.com/jakeykj/CareBench 找到。
