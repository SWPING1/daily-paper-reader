...
score: 9.0
evidence: 通过交叉注意力与时序建模进行多模态情感识别
...
## 摘要
由于面部外观、头部姿态、光照、背景噪声的巨大变化以及人类情感固有的动态特性，野外（in-the-wild）视频数据中的情感识别仍然是一个具有挑战性的问题。仅依赖单一模态（如面部表情或语音）通常不足以捕捉这些复杂的情感线索。为了解决这一问题，我们针对第10届野外情感行为分析（ABAW）挑战赛中的表情（EXPR）识别任务，提出了一种多模态情感识别框架。我们的方法利用大规模预训练模型，即用于视觉编码的 CLIP 和用于音频表示学习的 Wav2Vec 2.0，作为冻结的主干网络。为了建模面部表情序列中的时序依赖关系，我们在固定长度的视频窗口上采用了时序卷积网络（TCN）。此外，我们引入了一个双向交叉注意力融合模块，其中视觉和音频特征对称地进行交互，以增强跨模态上下文关联并捕捉互补的情感信息。随后使用一个轻量级分类头进行最终的情感预测。我们进一步结合了基于 CLIP 文本特征的文本引导对比目标，以促进语义对齐的视觉表示。在 ABAW 第10届 EXPR 基准测试上的实验结果表明，所提出的框架提供了一个强大的多模态基准，并取得了优于单模态建模的性能。这些结果证明了结合时序视觉建模、音频表示学习和跨模态融合对于在不受限的真实世界环境中实现鲁棒情感识别的有效性。
