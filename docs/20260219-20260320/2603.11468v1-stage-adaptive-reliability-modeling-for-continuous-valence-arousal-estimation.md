...
## 摘要
由于模态可靠性的不一致以及视听信号中依赖于交互的可变性，真实环境中的连续效价-唤醒度估计具有挑战性。现有方法主要侧重于建模时间动态，往往忽略了模态可靠性在不同交互阶段可能发生显著变化这一事实。为了解决这一问题，我们提出了 SAGE，一个阶段自适应可靠性建模框架，该框架在多模态集成过程中显式地估计并校准各模态的置信度。SAGE 引入了一种可靠性感知融合机制，根据音频和视觉表示与阶段相关的信息量动态地重新平衡它们，从而防止不可靠信号主导预测过程。通过将可靠性估计与特征表示分离，所提出的框架能够在跨模态噪声、遮挡和变化的交互条件下实现更稳定的情感估计。在 Aff-Wild2 基准测试上的大量实验表明，与现有的多模态融合方法相比，SAGE 一致地提高了一致性相关系数得分，突显了可靠性驱动建模在连续情感预测中的有效性。
