...
result: 在BraTS 2018和2020数据集上的实验结果显示，该方法在多模态和个性化联邦学习场景下均优于现有主流方法。
conclusion: FedMEPD通过创新的编码器-解码器设计和锚点校准机制，有效平衡了全局知识共享与局部个性化适配，显著提升了脑肿瘤分割的鲁棒性。
---

## 摘要
现有的用于医学图像分析的联邦学习（FL）方法大多仅考虑了模态内异质性，限制了它们在多模态成像应用中的适用性。在实践中，一些联邦学习参与者可能仅拥有完整成像模态的一个子集，这带来了模态间异质性，成为在所有参与者数据上有效训练全局模型的挑战。同时，每个参与者都希望在联邦学习中获得一个针对其本地数据特征定制的个性化模型。本研究提出了一种
